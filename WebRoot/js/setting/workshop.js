...
$(document).ready(function(){
	...
	$(document).on("click","#btnAdd",function(){
		var dialog = $( "#dialog-add" ).removeClass('hide').dialog({
			width:600,
			modal: true,
			title: '<div class="widget-header"><h4 class="smaller"><i class="ace-icon fa fa-gear green"></i> Add Workshop</h4></div>',
			title_html: true,
			buttons: [ 
				{
					text: "Cancel",
				...
				},
				{
					...
					click: function() {
						if($("#addWorkshopName").val()===""){
							alert("The workshop name cannot be empty！");
							$("#addWorkshopName").focus();
							return false;
						}
						if($("#addWorkshopCode").val()===""){
							alert("The workshop code cannot be empty！");
	...
	$(document).on("click",".editfactory",function(){
		...
		var dialog = $( "#dialog-edit" ).removeClass('hide').dialog({
			...
			/*height:500,*/
			modal: true,
			title: '<div class="widget-header"><h4 class="smaller"><i class="ace-icon fa fa-gear green"></i> Edit Workshop</h4></div>',
			title_html: true,
			buttons: [ 
				{
					text: "Cancel",
...
function ajaxDelete(){
	...
	if(ids===''){
		$.gritter.add({
			title: 'Message：',
			text: '<h5>At least one workshop must be checked！</h5>',
			class_name: 'gritter-info'
		});
		return false;
	}
	$.ajax({
	    ...
	    success:function(response){
	    	...
	    	$.gritter.add({
				title: 'Message：',
				text: '<h5>Delete workshop success！</h5>',
				class_name: 'gritter-info'
			});
	    	...
	    	ajaxQuery();
	    		...
	    		$.gritter.add({
					title: 'Message：',
					text: '<h5>Delete workshop failed！</h5><br>'+response.message,
					class_name: 'gritter-info'
				});
	    	}
	    }
...
function ajaxQuery(){
	$("#tableData").dataTable({
		...
		columns: [
		            ...
		            {"title":"Workshop Name","class":"center","data":"workshopName",/*"render": function ( data, type, row ) {
	                    return "<input style='border:0;width:100%;height:100%;background-color:transparent;text-align:center;' value='"+data+"' />";
	                },*/"defaultContent": ""},
		            {"title":"Workshop Code","class":"center","data":"workshopCode","defaultContent": ""},
