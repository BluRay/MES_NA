					...
					<form action="<%=basePath%>/snaker/flow/process" method="post">
						...
						<table id="tableData" class="table table-striped table-bordered table-hover dataTable no-footer"
							...
							<tr role="row" class="odd">
							    <td>&nbsp;资料需求节点：</td>
								<td>
									<input type="radio" value="OK" name="S_plandepNode"/>&nbsp;&nbsp;OK &nbsp;&nbsp;&nbsp;
							        <input type="radio" value="NG" name="S_plandepNode"/>&nbsp;&nbsp;NG &nbsp;&nbsp;&nbsp;
							        <input type="radio" value="待评审" name="S_plandepNode"/>&nbsp;&nbsp;待评审
								</td>
							</tr>
							<tr role="row" class="even">
								<td>&nbsp;其他：</td>
								<td colspan=2>
								<textarea id="plandepInfo" name="S_plandepInfo" style="width: 580px;" placeholder="..." rows="3"></textarea>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td></td>
								<td align="right">
									<input type="hidden" id="plandep_operator" name="S_plandep.operator" value="${operator }" />
						            <input type="hidden" id="result_operator" name="S_result.operator" value="${nextOperator }" />
		...
		<script type="text/javascript">
            $(function(){
            	$("#btnSave").click(function(){
            		if($("#result_operator").val()==""){
            			alert("暂未给用户分配【评审结果修正】角色,无法进行评审");
            			return false;
            		}
            		$.ajax({
            			...
            			dataType: "json",
            			data: {
            				"plandepNode" : $('input:radio[name="S_plandepNode"]:checked').val(),
            				"plandepInfo":$("#plandepInfo").val(),
