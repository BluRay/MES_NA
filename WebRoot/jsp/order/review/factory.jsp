					...
					<form action="<%=basePath%>/snaker/flow/process" method="post">
						...
						<table id="tableData" class="table table-striped table-bordered table-hover dataTable no-footer"
							...
							<tr role="row" class="odd">
							    <td>&nbsp;资料需求节点：</td>
								<td>
									<input type="radio" value="OK" name="S_factoryNode"/>&nbsp;&nbsp;OK &nbsp;&nbsp;&nbsp;
							        <input type="radio" value="NG" name="S_factoryNode"/>&nbsp;&nbsp;NG &nbsp;&nbsp;&nbsp;
							        <input type="radio" value="待评审" name="S_factoryNode"/>&nbsp;&nbsp;待评审
								</td>
							</tr>
							<tr role="row" class="even">
								<td>&nbsp;其他：</td>
								<td colspan=2>
								<textarea id="factoryInfo" name="S_factoryInfo" style="width: 580px;" placeholder="..." rows="3"></textarea>
								</td>
							</tr>
							<tr role="row" class="odd">
								<td></td>
								<td align="right">
									<input type="hidden" id="planning_operator" name="S_planning.operator" value="${nextOperator }" />
						            <input type="hidden" id="factory_operator" name="S_factory.operator" value="${operator }" />
		...
		<script type="text/javascript">
            $(function(){
            	$("#btnSave").click(function(){
            		if($("#planning_operator").val()==""){
            			alert("暂未给用户分配【综合计划部物控评审】角色,无法进行评审");
            			return false;
            		}
            		$.ajax({
            			...
            			dataType: "json",
            			data: {
            				"factoryNode" : $('input:radio[name="S_factoryNode"]:checked').val(),
            				"factoryInfo":$("#factoryInfo").val(),
            				"factory.operator" : $("#factory_operator").val(),
